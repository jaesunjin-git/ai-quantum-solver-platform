...
### 절대 금지 (NEVER)
- 코드 내에 도메인 특화 상수를 하드코딩하지 마라 (set 이름, 파라미터 값, 임계값 등)
- 특정 도메인(railway, crew 등)의 이름이나 값이 engine/ 내부에 나타나면 안 된다
- 새로운 case를 추가할 때 if/elif 분기로 처리하지 마라

### 필수 원칙 (ALWAYS)
- 상수는 반드시 YAML config 또는 데이터에서 로딩하라
- 새로운 분류/유형이 추가될 때 코드 변경 없이 config 추가만으로 동작해야 한다
- 판단 기준은 "이 값이 바뀌면 코드를 수정해야 하는가?" — Yes이면 외부화하라

### 설계 패턴
- engine/ 레이어: 도메인 무관, config/data 기반 동작
- domains/ 레이어: 도메인 전용 로직, engine을 상속하여 override
- knowledge/ 레이어: YAML 선언, 코드 변경 없이 도메인 확장 가능

### 자동 주입/확장 패턴
- 새로운 항목 추가 시 "기존 데이터/config에서 자동 판별" 가능한 방법을 먼저 설계하라
- 불가능한 경우에만 config에 명시적 선언을 추가하라
- config에도 넣을 수 없는 경우에만 코드에 넣되, 반드시 주석으로 사유를 기록하라

### 검증 기준
- 코드 리뷰 시 다음을 자문하라:
  - "다른 도메인에서도 이 코드가 그대로 동작하는가?"
  - "새로운 유형 추가 시 이 파일을 수정해야 하는가?"
  - 둘 중 하나라도 No이면 설계를 재고하라
- **REFACTOR 시 반드시 이전 실행 결과(column 수, type 분포, solver status)와 비교하여 동작 보존 확인**
