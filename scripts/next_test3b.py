...
# TEST 3b: test2 + cleanup_time만
test_names = ['trip_coverage', 'max_driving_time', 'max_work_time', 'cleanup_time']
base_model['constraints'] = [c for c in all_constraints if c.get('name') in test_names]

print(f'=== TEST 3b: + cleanup_time only ===')
for c in base_model['constraints']:
    if c.get('name') == 'cleanup_time':
